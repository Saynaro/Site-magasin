...
export const showProducts = async (req, res) => {
    ...
    try {
        ...
        const { subcategory, minPrice, maxPrice, sortBy } = req.query;
        let queryCondition = {};

        if (subcategory) {
            queryCondition.category = {
                name: subcategory
            };
        }
        ...
        if (sortBy === 'price_asc') {
            orderCondition = { priceCents: 'asc' };
        } else if (sortBy === 'price_desc') {
            orderCondition = { priceCents: 'desc' };
        ...
        if (products.length === 0) {
            return res.status(404).json({ 
                error: "No products find in database!"
            });
        };
